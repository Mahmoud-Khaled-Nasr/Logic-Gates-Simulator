...
void NAND3::Save(std::fstream& File) const
{
    File.clear();
    File.seekg(0);
    ...
    File.clear();
    File.seekp(0);
    File << std::setw(15) << std::left << "NAND3" << std::setw(5) << std::left << GetGraphicsInfo().GetX() << std::setw(5) << std::left << GetGraphicsInfo().GetY() << std::setw(5) << std::left << GetGraphicsInfo().GetWidth() << std::setw(5) << std::left << GetGraphicsInfo().GetHeight() << GetLabel() << std::endl;
    File << dummy;
}
